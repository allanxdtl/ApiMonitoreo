            ...
            $.get(`${apiBase}/Cliente/Buscar/${nombre}`, function (data) {
                ...
                if (data.length > 0) {
                    data.forEach(cliente => {
                        const clienteDiv = $(`<div class="resultado-cliente" data-id="${cliente.id}" data-nombre="${cliente.razonSocial}">${cliente.razonSocial}</div>`);
    ...
    $("#formO").on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                Swal.fire("Éxito", "Orden creada correctamente. El PDF se ha descargado.", "success");
            },
            error: function (xhr) {
                let errorMsg = xhr.responseText || "Error al crear la orden. Intente de nuevo.";
